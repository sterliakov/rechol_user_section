...
type NullToUndefined<T> = {[K in keyof T]: T[K] extends unknown | null ? Exclude<T[K], null>|undefined : T[K]}
export function getErrors<T extends MinimalForm>(response: any): NullToUndefined<T | MinimalForm> {
	...
	for (const [field, value] of Object.entries(response)) {
		if (field === 'errors') {
			...
			errors = {...errors, ...nestedErrors, submit: [errors.submit, nestedErrors.submit].filter(Boolean).join("; ") || null};
	...
	return Object.fromEntries(Object.entries(errors).map(([k,v])=>[k, v == null ? undefined : v])) as any;
}
