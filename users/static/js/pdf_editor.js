// biome-ignore lint/correctness/noUnusedVariables: referenced in module
function renderPdfEditor(
  ...
  class MyStoreAdapter extends PDFJSAnnotate.StoreAdapter {
    ...
    async __deleteAnnotation(_documentId, annotationId) {
      await $.post({
        url: urls.annotationDetail.replace('__unknown__', annotationId),
        method: 'DELETE',
        headers: { 'X-CSRFToken': csrfmiddlewaretoken },
    ...
    async getComments(_documentId, _annotationId) {
      return [];
    }
    async __addComment(_documentId, _annotationId, _content) {
      throw new Error('Not implemented!');
    }
    async __deleteComment(_documentId, _commentId) {
      throw new Error('Not implemented!');
    }
  }
  ...
  const RENDER_OPTIONS = {
    documentId,
    pdfDocument: null,
    scale: Number.parseFloat(localStorage.getItem(`${documentId}/scale`), 10) || 1.33,
    rotate: Number.parseInt(localStorage.getItem(`${documentId}/rotate`), 10) || 0,
  };
  ...
  function render() {
    ...
    loadingTask.promise.then((pdf) => {
      ...
      UI.renderPage(1, RENDER_OPTIONS).then(([pdfPage]) => {
  ...
  (() => {
    ...
    initColorPicker(document.querySelector('.hotspot-color'), hotspotColor, (value) => {
  ...
  (() => {
    ...
    function initText() {
      ...
      for (const s of [8, 9, 10, 11, 12, 14, 18, 24, 30, 36, 48, 60, 72, 96]) {
      ...
      initColorPicker(document.querySelector('.text-color'), textColor, (value) => {
  ...
  (() => {
    ...
    function initPen() {
      ...
      initColorPicker(document.querySelector('.pen-color'), penColor, (value) => {
  ...
  // Scale/rotate
  (() => {
    function setScaleRotate(scaleStr, rotateStr) {
      const scale = Number.parseFloat(scaleStr, 10);
      const rotate = Number.parseInt(rotateStr, 10);
